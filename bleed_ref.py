...
def fit_basis_functions(Reads, tissue_mask, Rates, global_rates, basis_idxs, basis_mask, lam=0, local_weight=100, x_init=None):
    ...
    def loss(t_Betas):
        ...
        L = -torch.stack([Multinomial(total_count=N[i], probs=t_Mu[:,i]).log_prob(t_Y[i]) for i in range(Reads.shape[1])], dim=0).mean()
...
def fit_spot_rates(Reads, tissue_mask, Weights, x_init=None):
    ...
    def loss(t_Rates):
        ...
        # Calculate the negative log-likelihood of the data
        L = -torch.stack([Multinomial(total_count=N[i], probs=Mu[:,i]).log_prob(t_Y[i]) for i in range(Reads.shape[1])], dim=0).mean()
