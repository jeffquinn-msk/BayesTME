    ...
    for g in range(Rates.shape[1]):
        ...
        for i, ((y, x), (dy, dx)) in enumerate(zip(locations, Vectors)):
            plt.arrow(x, y, dx, dy, width=0.1 * np.sqrt(dx ** 2 + dy ** 2), head_width=0.2 * np.sqrt(dx ** 2 + dy ** 2),
                      color='black', alpha=np.sqrt(dx ** 2 + dy ** 2))
...
def fit_spot_rates(Reads, tissue_mask, Weights, x_init=None):
    ...
    if x_init is None:
        x_init = np.concatenate(
            [np.median(Reads, axis=0), np.copy(Reads[tissue_mask].reshape(-1) * 1.1).clip(1e-2, None)])
...
def select_local_weight(Reads, tissue_mask, basis_idxs, basis_mask,
    ...
    for widx, local_weight in enumerate(weight_grid):
        res = decontaminate_spots(train[0], train[1], train[2], train[3], local_weight=local_weight,
                                  basis_init=basis_init, Rates_init=Rates_init, n_top=n_top)
...
if __name__ == '__main__':
    ...
    best_local_weight, delta_local_weight, lw_losses, lw_grid, (best_basis_init, best_Rates_init) = select_local_weight(
        Reads, tissue_mask, basis_idxs, basis_mask)
    ...
    plt.plot(lw_grid, np.array([lw_grid ** 2, lw_grid]).T.dot(loss_curve_coefs[:2]) + loss_curve_coefs[2],
             color='lightblue')
    ...
    global_rates, fit_Rates, basis_functions, Weights, basis_init, Rates_init = decontaminate_spots(Reads, tissue_mask,
                                                                                                    basis_idxs,
                                                                                                    basis_mask,
    ...
    with sns.axes_style('white'):
        ...
        for i in range(min(n_genes, 3)):
            ...
            fig, axarr = plt.subplots(3 + int(have_truth), 2, figsize=(5 * 2, 5 * (3 + int(have_truth))), sharex=True,
                                      sharey=True)
