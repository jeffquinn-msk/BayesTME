...
print('experiment: {}, lambda {}, {} components, fold {}, {} spots heldout'.format(exp_name, lam_psi, n_components, n_fold, len(heldout_spots)))
...
gfnb = GraphFusedMultinomial(n_components=n_components, edges=edges, Observations=train, n_gene=n_gene, lam_psi=lam_psi, 
...
for step in range(n_samples*n_thin+n_burn):
    ...
    if step >= n_burn and (step - n_burn) % n_thin == 0:
        ...
        loglhtest_trace[idx] = np.array([multinomial.logpmf(test[i], test[i].sum(), nb_probs[i]) for i in heldout_spots]).sum()
