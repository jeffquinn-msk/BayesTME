...
from scipy.sparse import diags, spdiags, eye, kron, csc_matrix, block_diag
import utils
...
#         self.path_nodes += (depth_sizes-1)[None]
    ...
    def resample(self, outcomes):
        ...
        self.Sigma0_inv = kron(np.eye(self.n_classes-1), self.Delta.T.tocsc().dot(self.lam_Tau).dot(self.Delta)).tocsc()
        ...
        self.Thetas = utils.sample_mvn_from_precision(self.Sigma_inv, mu_part=self.mu, sparse=True, Q_shape=self.Sigma_inv.shape).reshape(self.Thetas.shape[1], self.Thetas.shape[0]).T
        ...
        self.Tau2_c = 1/np.random.gamma(1, 1 / (1/self.Tau2 + 1/self.Tau2_b).clip(self.stability, 1/self.stability))
        self.Tau2_b = 1/np.random.gamma(1, 1 / (1/self.Tau2_c + 1/self.Tau2_a).clip(self.stability, 1/self.stability))
