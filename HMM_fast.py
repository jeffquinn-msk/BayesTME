...
class HMM:
    ...
    def forward_filter(self, Obs):
        ...
        for i in range(self.n_states-1):
            ...
            # P(R_ik|d_ik)
            self.alpha_log[:, i] = self.ship_up(predictor_log) + emission_fast(Obs[:, i], self.log_cell, self.expression[i], self.cell_grid)[:, None]
        self.alpha_log[:, -1] += emission_fast(Obs[:, -1], self.log_cell, self.expression[-1], self.cell_grid)[:, :, None]
