...
class GraphFusedMultinomial:
    ...
    def sample_cell_num(self):
        ...
        with np.errstate(divide='ignore'):
            ...
            start_prob = np.array([binom.logpmf(np.arange(self.n_max+1), self.n_max, p=self.probs[i, 0]) for i in range(self.n_nodes)])
            self.Transition = transition_mat_vec(prob, self.n_max+1)
            if self.bkg:
                self.cell_num[:, :-1] = self.HMM.ffbs(np.transpose(self.reads[:, :, :-1], [0, 2, 1]), start_prob, LogTransition=np.log(self.Transition), expression=self.expression[:-1])
            else:
                self.cell_num = self.HMM.ffbs(np.transpose(self.reads, [0, 2, 1]), start_prob, LogTransition=np.log(self.Transition), expression=self.expression)
